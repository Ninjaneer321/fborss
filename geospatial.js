...
var feed = new RSS({
    ...
    copyright: today.getFullYear() + ' Bill Dollins',
    language: 'en',
...
archive.forEach(function(entry){
	feed.item({
		...
		url: entry.URL,
		date: getDateObject(entry.DATE, entry.YEAR)
	});
});
...
function formatFBOName(){
	...
	var date = dt.getDate(); //need yesterday
...
function getDateObject(day, year){
    var m = day.substring(0,2);
    var d = day.substring(day.length - 2);
